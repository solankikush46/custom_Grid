...
class GridWorldEnv(Env):
    def __init__(self):
        ...
        self.observation_space = Dict({
            ...
            "exit_distances": Box(low=0.0, high=1.0, shape=(3,), dtype=np.float32),  # normalized
        })
        ...
        # Compute radar zone
        self.sensor_radar_zone = setup_obstacles.compute_sensor_radar_zone(
            self.sensor_batteries.keys(), max(self.grid_width, self.grid_height)
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        self.sensor_radar_zone = setup_obstacles.compute_sensor_radar_zone(self.sensor_batteries.keys(), self.grid_height)
    ...
    def render_pygame(self):
        ...
        colors = {
            ...
            '*': (255, 255, 0),    # Trail outside radar
            'A': (0, 0, 255),      # Agent
            'G': (0, 255, 0),      # Goal
            'F': (0, 255, 255),    # Finished at goal
            'S': (255, 0, 0),      # Sensor
            'T': (173, 216, 230),  # Trail inside radar
            'BG': (255, 165, 0),   # Background radar zone (orange)
        }
        ...
        for pos in self.visited:
            ...
            if (x, y) != tuple(self.agent_pos) and (x, y) not in self.goal_positions:
                if self.static_grid[x, y] != 'S':
                    if (x, y) in self.sensor_radar_zone:
                        grid_copy[x, y] = 'T'  # trail in radar zone (pink)
                    else:
                        grid_copy[x, y] = '*'  # trail outside radar (yellow)
        ...
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                ...
                pygame.draw.rect(screen, colors['.'], (j * cell_size, i * cell_size, cell_size, cell_size))

                # Then radar zone (orange) if applicable (and not obstacle)
                if (i, j) in self.sensor_radar_zone and self.static_grid[i, j] != '#':
                    pygame.draw.rect(screen, colors['BG'], (j * cell_size, i * cell_size, cell_size, cell_size))
        ...
        for (x, y), battery in self.sensor_batteries.items():
            pygame.draw.rect(screen, colors['S'], (y * cell_size, x * cell_size, cell_size, cell_size))
            label = font.render(f"{int(battery)}", True, (0, 0, 0))
            text_rect = label.get_rect(center=(y * cell_size + cell_size // 2, x * cell_size + cell_size // 2))
