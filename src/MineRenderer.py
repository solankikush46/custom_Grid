...
from .constants import (
    EMPTY_CHAR, OBSTACLE_CHAR, SENSOR_CHAR, AGENT_CHAR, BASE_STATION_CHAR, MINER_CHAR, GOAL_CHAR,
    OBSTACLE_ID, # Import the integer ID for logic
    RENDER_COLORS
)
...
class MineRenderer:
    ...
    def render(self, static_grid, world_state):
        ...
        self._draw_agent(world_state.get('agent_pos'))
    ...
    def _draw_agent(self, agent_pos):
        if agent_pos:
            r, c = agent_pos
            pygame.draw.rect(self.screen, RENDER_COLORS[AGENT_CHAR], (c * self.cell_size, r * self.cell_size, self.cell_size, self.cell_size))
